...
**
** https://github.com/eliben/code-for-blog/blob/master/2011/dwarf_get_func_addr.c
*/
#include <stdio.h>
#include <stdarg.h>
#include <stdlib.h>
...
void list_func_in_die(Dwarf_Debug dgb, Dwarf_Die the_die)
{
    ...
    for (int i = 0; i < attrcount; ++i)
    {
        ...
        */
        const char *attrcode_message;
        int util_tc = -1;
        util_tc = get_DWART_AT_string(attrcode, &attrcode_message);
        if (util_tc == 0)
        {
            printf(" attrs[%d]: attrcode 0x%04x %s\n", i, attrcode, attrcode_message);
        }
    ...
    printf("low pc  : %016llx\n", lowpc);
    printf("high pc : %016llx\n", highpc);
}
