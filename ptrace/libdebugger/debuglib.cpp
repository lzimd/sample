...
void run_target(const char *programname)
{
    ...
    int ret = execl(programname, programname, NULL);
    if (ret == -1)
    {
        procmsg("target run failed. errno(%d) - %s\n", errno, strerror(errno));
    }
}

uint64_t get_child_eip(pid_t pid)
{
...
void dump_process_memory(pid_t pid, uint64_t from_addr, uint64_t to_addr)
{
    ...
    for (uint64_t addr = from_addr; addr <= to_addr; ++addr)
    {
        ...
        unsigned long word = ptrace(PTRACE_PEEKTEXT, pid, (void *)addr, 0);
        printf("%02lx ", word & 0xFF);
        index++;
        if (index == dumpLine)
        {
            printf("\n");
...
{
    debug_breakpoint *bp = (debug_breakpoint *)malloc(sizeof(struct debug_breakpoint_t));
    bp->addr = addr;
